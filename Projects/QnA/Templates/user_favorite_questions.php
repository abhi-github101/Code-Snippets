...
$cres=mysqli_query($con,"select count(*) as 'count' from user_fav_question;");
$cdat=mysqli_fetch_array($cres,MYSQLI_ASSOC);
$rowsCount=$cdat["count"];

$fres=mysqli_query($con,"select qid from user_fav_question order by CONVERT(SUBSTR(qid,2),SIGNED INTEGER) desc limit ".$start_limit.",".$fetchRows.";");
$userFavQuestions=mysqli_num_rows($fres);
if($userFavQuestions>0){
$formid=1;

while($fdata=mysqli_fetch_array($fres,MYSQLI_ASSOC)){
$rs=mysqli_query($con,"select qid,title,details,attachment,tag,custom_tags,status,views,post_date from questions_table where qid='".$fdata["qid"]."';");
